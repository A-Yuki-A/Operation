...
# Sidebar: input and PC display
with st.sidebar:
    st.header("入力パネル")
    A = st.number_input("値Aを入力 (番地7 に格納)", value=0)
    B = st.number_input("値Bを入力 (番地8 に格納)", value=0)
    # Step and PC calculation
    step = st.session_state.step
    pc = 1 if step < 3 else math.ceil((step - 1) / 2)
    st.markdown(f"**プログラムカウンタ :** <span style='font-size:32px'>{pc}</span>", unsafe_allow_html=True)
...
# Left: CPU & memory diagram
with col1:
    st.subheader("各装置の関係図 (動作中)")
    # Build HTML table for memory
    rows = ""
    for addr in ['1','2','3','4','5','6','7','8','9']:
    ...
    digraph G {{
      ...
      subgraph cluster_ctrl {{
        ...
        IR [label="命令レジスタ: {data.get(str(1))}"];
        Decoder [label="命令解読器"];
